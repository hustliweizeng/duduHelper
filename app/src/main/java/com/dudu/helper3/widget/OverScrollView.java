 ...
 * <br>
 * <strong>策略:</strong> 获取ScrollView的子视图并添加到自定义的{@link OverScrollWarpLayout}
 * 滚动视图中 将滚动视图添加到ScrollView作为子视图,所有的弹性滚动都由{@link OverScrollWarpLayout}来完成
 * 
 * @author King
 */
public class OverScrollView extends ScrollView {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (isRecord) {
				...
				if (mOverScrollTinyListener != null) {
					mOverScrollTinyListener.scrollDistance(deltaY,
							overScrollDistance);
				}
				return true;
			}
	...
	private void initOverScrollLayout() {
		...
		if (mContentLayout == null) {
			...
			this.addView(mContentLayout, new LayoutParams(
