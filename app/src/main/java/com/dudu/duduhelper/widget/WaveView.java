...
public class WaveView extends View {
	...
	public static final int DEFAULT_BEHIND_WAVE_COLOR = Color
			.parseColor("#28FFFFFF");
	public static final int DEFAULT_FRONT_WAVE_COLOR = Color
			.parseColor("#3CFFFFFF");
	...
	private void init() {
		...
		if(getHeight()==0)
		{
		   height = Util.dip2px(context, 200);
		   int a = 2;
		   int b = a;
		}
		else
	 ...
	 *            Should be 0 ~ 1. Default to be 0. <br/>
	 ...
	 *            Should be 0 ~ 1. Default to be 0.5. <br/>
	 ...
	 *            Default to be 0.05. Result of amplitudeRatio + waterLevelRatio
	...
	/**
	 * Create the shader with default waves which repeat horizontally, and clamp
	 * vertically
	 */
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mShowWave && mWaveShader != null) 
		{
			...
			// this decides the size(mWaveLengthRatio for width, mAmplitudeRatio
			// for height) of waves
			mShaderMatrix.setScale(
					mWaveLengthRatio / DEFAULT_WAVE_LENGTH_RATIO,
					mAmplitudeRatio / DEFAULT_AMPLITUDE_RATIO, 0,
					mDefaultWaterLevel);
			// translate shader according to mWaveShiftRatio and
			// mWaterLevelRatio
			// this decides the start position(mWaveShiftRatio for x,
			// mWaterLevelRatio for y) of waves
			mShaderMatrix.postTranslate(mWaveShiftRatio * width,
			...
			case CIRCLE:
				if (borderWidth > 0) 
				{
					canvas.drawCircle(width / 2f, height / 2f,(width - borderWidth) / 2f - 1f, mBorderPaint);
				}
			...
			case SQUARE:
				if (borderWidth > 0) 
				{
					canvas.drawRect(borderWidth / 2f, borderWidth / 2f,width - borderWidth / 2f - 0.5f, height - borderWidth / 2f - 0.5f, mBorderPaint);
				}
				canvas.drawRect(borderWidth, borderWidth, width - borderWidth, height - borderWidth, mViewPaint);
				break;
			}
		} 
		else 
